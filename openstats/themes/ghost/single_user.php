...
<table>
  ...
  <table style="border:none; border:0; font-family: Arial, Verdana; font-size:12px;"> 
     <tr>
	   <td width="150">  <?=OS_ShowGravatar( $User["player"])?></td>
	   <td>
	   	 <div><b>Games:</b> <?=$User["totalgames"]?> </div>
		 <div><b>W/L:</b> <?=$User["totalwins"]?> / <?=$User["totallosses"]?> (<?=$User["totalwinslosses"]?>%) </div>
	   	 <div><b>Level:</b> <?=$User["level"]?> </div>
	   	 <div><b>Exp:</b> <?=$User["exp"]?> / <?=$User["end"]?> (<?=$User["progress"]?>%) </div>
		 
	     <div style="width:100px; background-color: #FFF; border:1px solid #9BA4C9; height: 14px;"> 
		   <div style="width: <?=$User["percent"]?>px; background-color: #1BC31B; height: 14px;"> &nbsp; </div> 
	     </div>
		 <?php if($User["totalvotes"]>=1) { ?>
		 <div><b>Rating:</b> <?=OS_DrawStars($User["player_rate"], 20)?> (<?=$User["player_rate"]?>) </div>
		 <div><b>Total Votes:</b> <?=$User["totalvotes"]?></div>
		 <?php } ?>
	   </td>
	 </tr>
  </table>
	 ...
	 <?=DisplayGameTypePlayer($GameAliases, $UserOtherGames, $User["player"], $User["pid"])?></td>
   </tr>
   <tr>
    <td  class="padLeft">
	<?=$User["realm"]?>
	...
	<?=OS_AdminTools($User["pid"], $User["player"], $User["member"])?>
	</td>
   </tr>
   <tr>
 ...
 <?php
  ...
  <table class="Table500px">
      <tr class="row">
	  <th class="padLeft" width="140">
	  <!--<?=$lang["stats"] ?>: <?=$User["month"]?> / <?=$User["year"]?>-->
	  <input type="hidden" name="u" value="<?=$User["pid"]?>" />
	  <select name="m" id="m">
	    <?php for ($x=1; $x<=12; $x++) { 
		$s = "";
		if(isset($_GET["m"]) AND $_GET["m"] == $x ) $s='selected="selected"'; 
		else if(!isset($_GET["m"]) AND $x == date("n") ) $s='selected="selected"'; 
		?>
		 <option <?=$s?> value="<?=$x?>"><?=getMonthName($x)?></option>
		<?php } ?>
	  </select>
	  ...
	  <select name="y" id="y">
		...
		$s = "";
		if(isset($_GET["y"]) AND $_GET["y"] == $x )      $s='selected="selected"';
		else if(!isset($_GET["y"]) AND $x == date("Y") ) $s='selected="selected"'; 
		?>
		 <option <?=$s?> value="<?=$x?>"><?=$x?></option>
		<?php } ?>
	  </select>

	  </th>
	  <th width="160"><input type="submit" value="Display" class="menuButtons" /></th>
	  <th width="60"></th>
	  <th width="175"></th>
	  <th width="175"></th>
	  <th width="175"></th>
    ...
    <tr class="row">
	  ...
	  <td width="160"><?=$User["score"]?> (<?=$User["totalscore"]?>)</td>
    ...
    <tr class="row">
	  ...
	  <td width="160"><a href="<?=OS_HOME?>?games&amp;uid=<?=$User["pid"]?>"><?=$User["games"]?></a> (<?=$User["totalgames"]?>)</td>
    ...
    <tr class="row">
	  ...
	  <td width="160"><?=($User["maxstreak"])?> (<?=($User["totalstreak"])?>)</td>
	</tr>
...
<tr>
  <td>
     <div class="padTop aligncenter" align="center">
       <h2><a name="game_history" href="<?=OS_HOME?>?games&amp;uid=<?=$User["pid"]?>"><?=$lang["user_game_history"] ?></a></h2>
    </div>
  </td>
</tr>
</table>
  ...
  <?=DisplayGameTypes( $GameAliasesGames, 'u='.$User["pid"], 1, '#game_history')?>
  ...
  <tr class="row GameHistoryRow">
	 ...
	 <?php if (isset($_GET["u"]) ) { ?>
	 <td width="40" height="40"><?=OS_UserHeroHistoryLink($User["pid"], $Games["hero"], $lang["show_hero_history"]) ?></td>
	 <td width="90">
