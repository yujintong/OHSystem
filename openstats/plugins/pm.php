...
if ($PluginEnabled == 1  ) {
    ...
    $AdminPMEmail = OS_SEND_PM_EMAIL;
	  
     if ( isset($_POST["AdminPMEmail"]) ) {
     $AdminPMEmail = (int)$_POST["AdminPMEmail"];
   
      write_value_of('$SendEmailOnPM', "$SendEmailOnPM", $AdminPMEmail , $plugins_dir.basename(__FILE__, '') ) ;
     }
...
if ( OS_is_admin() AND OS_PluginEdit( $ThisPlugin ) ) {
	 ...
	 $sel = array();
	 if ($AdminPMEmail == 1) $sel[0] = 'selected="selected"'; else $sel[0] = "";
	 if ($AdminPMEmail == 0) $sel[1] = 'selected="selected"'; else $sel[1] = "";

   //Show following options when user click on edit icon for this plugin
   $Option = '
<form action="" method="post" >
  On new Private Message: <br />
  <select name="AdminPMEmail">
    <option '.$sel[0].' value="1">Notify user via email</option>
	<option '.$sel[1].' value="0">Do nothing</option>
  </select>
  <input type="submit" value = "Save changes" class="menuButtons" />
  <a href="'.$website.'adm/?plugins" class="menuButtons">Cancel</a>
	...
	if ( isset($_GET["member"]) AND is_numeric($_GET["member"])  AND is_logged() ) {
	   AddEvent("os_custom_user_fields",  "OS_SendPMLinkMember"); 
	}
...
<div class="clr"></div>
 <div class="ct-wrapper"  id="content" class="s-c-x">
  <div class="outer-wrapper wrapper">
   ...
   function OS_SendPMLinkMember() {
	global $Member;
    ?>
	<tr>
	<td class="padLeft"><b>Private Message:</b></td>
	<td><a class="menuButtons" href="<?=OS_HOME?>?action=pm&amp;send=<?=$Member["user_id"]?>"> Send PM </a></td>
	</tr>
	<?php
   }
   ...
   <div class="content section" id="main-column">
    <div class="widget Blog padding">
     <div class="blog-posts hfeed padLeft padTop padBottom inner">
		...
		if (!isset($_GET["m"]) ) $text = limit_words( convEnt($row["field_value"]), 40); 
