...
if ($PluginEnabled == 1  ) {
   ...
   function OS_JQuery182() {
	 ...
	 if ( isset($_GET["showgames"]) AND $_GET["showgames"] == 200) $Total = 200;

	 $order = " DESC";
	 $label  = 'last';
	 
	 if ( isset($_GET["first"]) ) { $order = ' ASC'; $label  = 'first'; }
	 ...
	 WHERE s.id = :id AND (g.map) LIKE ('%".OS_DEFAULT_MAP."%') AND g.duration>='".$MinDuration."' 
	 ORDER BY g.id $order
	 LIMIT $Total");
...
jQuery(function () {
        jQuery('#chart_container').highcharts({
            ...
            title: {
                text: '<?=strtoupper($UserData[0]["player"])?> games <?=$order?>',
                x: -30 //center
            },
            subtitle: {
