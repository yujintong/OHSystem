...
if ( isset($_GET["edit"]) AND is_numeric($_GET["edit"]) ) {
   ...
   $row = $sth->fetch(PDO::FETCH_ASSOC);
	...
	//REPLAY
     ...
     //DELETE REPLAY
     if (isset($_GET["del_replay"]) AND OS_IsRoot() ) {
	...
	<?php if (isset( $Replay) AND !empty($Replay ) ) { ?>
	 ...
	 <a href="javascript:;" <?php if ( !OS_IsRoot() ) { ?>onclick="alert('You don\'t have permission to delete replay?')"<?php } else { ?>onclick="if (confirm('Delete replay?') ) {  location.href='<?=OS_HOME?>adm/?games&amp;edit=<?=$gameid?>&amp;del_replay' }" <?php } ?>>Delete replay?</a>
	 <?php } ?>
...
<?php
  ...
  <tr class="row" style="height:30px;">
	...
	<?php if (isset( $Replay) AND !empty($Replay ) ) { ?>
	 ...
	 <a href="javascript:;" <?php if (OS_IsRoot() ) { ?>onclick="if (confirm('Delete Game? (Note: After delete you\'ll have to reset and update the stats)') ) { location.href='<?=OS_HOME?>adm/?games&amp;del=<?=$row["id"]?>' }"<?php } else { ?>onclick="alert('You don\'t have permission to delete the game?')"<?php } ?>><img src="<?=OS_HOME?>adm/del.png" alt="img" /></a>
	 </td>
	<td width="80" class="font12"><?=secondsToTime($row["duration"])?></td>
	<td width="40" class="font12"><?=OS_GetGameState($row["type"])?></td>
