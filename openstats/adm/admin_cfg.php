...
if ( isset( $_POST["update_config"]) AND OS_IsRoot() ) {
   ...
   write_value_of('$MembersPerPage', "$MembersPerPage", trim( (int) $_POST["MembersPerPage"]) , "../config.php");
   ...
   //LIVE GAMES
   write_value_of('$LiveGames', "$LiveGames", trim( (int) $_POST["LiveGames"]) , "../config.php");
   write_value_of('$LiveGamesTotalLogs', "$LiveGamesTotalLogs", trim( (int) $_POST["LiveGamesTotalLogs"]) , "../config.php");
   
   write_value_of('$LiveShowAegis', "$LiveShowAegis", trim( (int) $_POST["LiveShowAegis"]) , "../config.php");
   write_value_of('$LiveShowSwaps', "$LiveShowSwaps", trim( (int) $_POST["LiveShowSwaps"]) , "../config.php");
   write_value_of('$LiveShowHeroPicks', "$LiveShowHeroPicks", trim( (int) $_POST["LiveShowHeroPicks"]) , "../config.php");
   write_value_of('$LiveShowPlayerLevels', "$LiveShowPlayerLevels", trim( (int) $_POST["LiveShowPlayerLevels"]) , "../config.php");
   write_value_of('$LiveShowKills', "$LiveShowKills", trim( (int) $_POST["LiveShowKills"]) , "../config.php");
   write_value_of('$LiveShowAssists', "$LiveShowAssists", trim( (int) $_POST["LiveShowAssists"]) , "../config.php");
   write_value_of('$LiveShowSuicides', "$LiveShowSuicides", trim( (int) $_POST["LiveShowSuicides"]) , "../config.php");
   write_value_of('$LiveShowRunes', "$LiveShowRunes", trim( (int) $_POST["LiveShowRunes"]) , "../config.php");
   write_value_of('$LiveShowTowerKills', "$LiveShowTowerKills", trim( (int) $_POST["LiveShowTowerKills"]) , "../config.php");
...
<a class="menuButtons" href="<?=OS_HOME?>adm/?cfg#live">LiveGames</a>
...
<form action="" method="post">
	...
	<tr>
	  <th width="240"></th>
	  <th class="padLeft"><a name="basic" class="anchor" href="#basic"></a>Basic Configuration</th>
	</tr>
	 ...
	 <tr class="row">
	   <td>Main Bot Name:</td>
	   <td ><input class="field" type="text" value="<?=$BotName?>" name="BotName" /></td>
	 </tr>
...
?>
	 ...
	 <tr  class="row">
	   <td>Login points:</td>
	   <td>
	   <input style="width:40px;" class="field" type="text" value="<?=$DailyPoints?>" name="DailyPoints" /> PTS
	   <div>Number of points that ranked members will get every 24 hours when they login on website. 0 to disable.</div>
	   </td>
	 </tr>
	 ...
	 <tr  class="row">
	   <td><div class="padTop"></div>Save Admin Logs:<div class="padTop"></div></td>
	   <td >
	   <div class="padTop"></div>
	   <select name="SaveAdminLogs">
		 <?php if ( $SaveAdminLogs == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Yes</option>
		 <?php if ( $SaveAdminLogs == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">No</option>
	   </select>
	   <span class="font12">Record the activities of admins in the database</span>
	    <div class="padTop"></div>
	   </td>
	 </tr>
	 
	 <tr class="row">
	   <td><div class="padTop"></div>Online/offline users on Top page:</div></td>
	   <td>
	   <div class="padTop"></div>
	   <select name="OnlineOfflineOnTopPage">
	   <?php if ($OnlineOfflineOnTopPage==1) $sel='selected="selected"'; else $sel = ""; ?>
	     <option <?=$sel?> value="1">Enabled</option>
	   <?php if ($OnlineOfflineOnTopPage!=1) $sel='selected="selected"'; else $sel = ""; ?>
	     <option <?=$sel?> value="0">Disabled</option>
	   </select>
	   (Display online/offline users on Top page. Require additional queries.) 
	   <div class="padTop"></div>
	   </td>
	 </tr>
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="live" class="anchor" href="#live"></a>Live Games</th>
	</tr>
	
	 <tr class="row">
	   <td>Live Games Page:</td>
	   <td>
	   <select name="LiveGames">
		 <?php if ( $LiveGames == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Enabled</option>
		 <?php if ( $LiveGames == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
	   </select>
	   </td>
	 </tr>
	 ...
	 <tr class="row">
	   <td>Keep</td>
	   <td><input style="width:60px;" class="field" type="text" value="<?=$LiveGamesTotalLogs?>" name="LiveGamesTotalLogs" /> log records 
	   <a href="<?=OS_HOME?>adm/?live_games">View logs</a> | 
	   <a href="javascript:;" onclick="if (confirm('Purge all logs?') ) { location.href='<?=OS_HOME?>adm/?live_games&purge_logs' }">Purge logs</a>
	   <div>After <b><?=$LiveGamesTotalLogs?></b> entries, all data will be purged. 0 - disable (note: can drastically affect the size of the database). </div>
	   </td>
	 </tr>
	 ...
	 <tr class="row">
	   ...
	   <td>
	   <div>
	   <select name="LiveShowAegis">
		 <?php if ( $LiveShowAegis == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowAegis == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowAegis == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Aegis</b> picks
	   </div>
	   
	   <div>
	   <select name="LiveShowSwaps">
		 <?php if ( $LiveShowSwaps == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowSwaps == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowSwaps == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Swaps</b>
	   </div>
	   
	   <div>
	   <select name="LiveShowHeroPicks">
		 <?php if ( $LiveShowHeroPicks == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowHeroPicks == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowHeroPicks == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Hero picks</b>
	   </div>
	   
	   <div>
	   <select name="LiveShowPlayerLevels">
		 <?php if ( $LiveShowPlayerLevels == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowPlayerLevels == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowPlayerLevels == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Player Levels</b>
	   </div>
	   
	   
	   <div>
	   <select name="LiveShowKills">
		 <?php if ( $LiveShowKills == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowKills == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowKills == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Player Kills</b>
	   </div>
	   
	   <div>
	   <select name="LiveShowAssists">
		 <?php if ( $LiveShowAssists == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowAssists == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowAssists == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	   </select> - Show <b>Player Assists</b> (only if player kills enabled)
	   </div>
	   
	   <div>
	   <select name="LiveShowSuicides">
		 <?php if ( $LiveShowSuicides == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowSuicides == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowSuicides == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Suicides</b>
	   </div>
	   
	   <div>
	   <select name="LiveShowTowerKills">
		 <?php if ( $LiveShowTowerKills == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowTowerKills == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowTowerKills == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Tower</b> kills
	   </div>
	   
	   <div>
	   <select name="LiveShowRoshanKill">
		 <?php if ( $LiveShowRoshanKill == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowRoshanKill == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowRoshanKill == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Roshan</b> kills
	   </div>
	   
	   <div>
	   <select name="LiveShowRunes">
		 <?php if ( $LiveShowRunes == 1 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
	     <option <?=$sel?> value="1">Always show</option>
		 <?php if ( $LiveShowRunes == 0 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="0">Disabled</option>
		 <?php if ( $LiveShowRunes == 2 ) $sel = 'selected="selected"'; else $sel = ""; ?> 
		 <option <?=$sel?> value="2">Only for registered users</option>
	   </select> - Show <b>Runes</b> picks
	   </div>
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="integration" class="anchor" href="#integration"></a>Integration</th>
	</tr>
...
<?php if ($WPIntegration == 1 AND !empty($wperror)) { $class='class="sentinel"'; }  ?>
	 ...
	 <tr  class="row">
	   ...
	   <div id="wphelp" style="display:none; color: #585858;">
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="ban" class="anchor" href="#ban"></a>Ban Reports/Appeals</th>
	</tr>
...
<?php if ($ReportUserLink == 0) $sel = 'selected="selected"'; else $sel = ""; ?>
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="stats" class="anchor" href="#stats"></a>Stats</th>
	</tr>
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="compare" class="anchor" href="#compare"></a>Compare players</th>
	</tr>
...
<?php if ($ComparePlayers == 0) $sel = 'selected="selected"'; else $sel = ""; ?>
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="pages" class="anchor" href="#pages"></a>Pages</th>
	</tr>
	 ...
	 <tr  class="row">
	   <td>Members per page:</td>
	   <td ><input style="width: 98px;" class="field" type="text" value="<?=$MembersPerPage?>" name="MembersPerPage" /></td>
	 </tr>
...
-->
	...
	<tr>
	  <th></th>
	  <th  class="padLeft"><a name="misc" class="anchor" href="#misc"></a>Misc</th>
	</tr>
