...
<?php
  ...
  if ( isset($_GET["edit"]) AND isset($_GET["t"]) AND is_numeric($_GET["t"]) ) {
	...
	$expiredate = time()+3600*24*7; //7 days ban by default
	
	$expire = date("Y-m-d H:i:00", $expiredate);
	
	$sth  = $db->prepare("INSERT INTO ".OSDB_BANS." (name, date, admin, reason, expiredate) VALUES('".$id."', '".$date."', '".$_SESSION["username"]."', 'Reported by other player', '".$expire."') ");  
	$result = $sth->execute();
	...
	$Country  = geoip_country_name_by_addr($GeoIPDatabase, $row["user_ip"]);
	 ...
	 <table class="Table800px">
	   ...
	   <tr>
		...
		<td><span><?php if (!empty($row["game_url"]) ) { ?><a href="<?=$row["game_url"]?>" target="_blank"><?=$row["game_url"]?></a><?php } else { ?><?=$row["game_url"]?><?php } ?></span></td>
	   </tr>
	   <tr>
	    <td width="150" class="padLeft"><b>Replay URL:</td>
		<td><span><?php if (!empty($row["replay_url"]) ) { ?><a href="<?=$row["replay_url"]?>" target="_blank"><?=$row["replay_url"]?></a><?php } else { ?><?=$row["replay_url"]?><?php } ?></span></td>
	   </tr>
	   <tr>
...
<?php
   ...
   <tr class="row font13" style="height: 55px;">
	 ...
	 <td><?=limit_words( convEnt($row["reason"]), 12)?></td>
