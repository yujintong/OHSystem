...
//delete
  if ( isset( $_GET["del"]) AND is_numeric($_GET["del"]) ) {
	  ...
	  ?>
	  <div align="center">
	  <h2>Note successfully deleted. <a href="<?=OS_HOME?>adm/?notes">&laquo; Back</a></h2>
	  </div>
	  <?php 
   ...
   //UPDATE
    if ( isset($_POST["edit_list"]) ) {
	  ...
	  if ( $result ) {
	  	  ?>
	  <div align="center">
	    <h2>Note successfully updated. <a href="<?=OS_HOME?>adm/?notes">&laquo; Back</a></h2>
	  </div>
	  <?php 
	  if ( isset($_GET["edit"]) ) OS_AddLog($_SESSION["username"], "[os_notes] Edited Note: ( $name ) ");
	  if ( isset($_GET["add"]) )  OS_AddLog($_SESSION["username"], "[os_notes] Added Note: ( $name ) ");
	  }
	 } else {
	?>
...
?>
<div align="center">
<div class="padBottom padTop"><a class="menuButtons" href="<?=OS_HOME?>adm/?notes&amp;add">[+] Add Note</a></div>
<?php
  ...
  include('pagination.php');
   ...
   <tr class="row" style="height:32px;">
     <td width="180" class="padLeft font12"><a href="<?=OS_HOME?>adm/?notes&amp;edit=<?=$row["id"]?>"><?=$row["name"]?></a></td>
	 <td width="64" class="font12">
	 <a href="<?=$website?>adm/?notes&amp;edit=<?=$row["id"]?>"><img src="<?=OS_HOME?>adm/edit.png" alt="img" /></a>
	 <a href="javascript:;" onclick="if (confirm('Delete Note?') ) { location.href='<?=OS_HOME?>adm/?notes&amp;del=<?=$row["id"]?>' }"><img src="<?=OS_HOME?>adm/del.png" alt="img" /></a>
	 </td>
