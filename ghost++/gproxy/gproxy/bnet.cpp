		...
		else
		{
			...
			if ( whisper )
			{
				if ( r == 3 || r == 9 )
				{
					...
					if (chatCommand.length() >=14 && nameEndpos != -1 && chatCommand.substr( 14, 10 ) != "gproxylist" )
					{
			...
			if( chatCommand.substr( 0, 1 ) == "!" )
			{
				string User = m_UserName;
			    string Command;
                string Payload;
                string :: size_type PayloadStart = chatCommand.find( " " );

                if( PayloadStart != string :: npos )
                {
					Command = chatCommand.substr( 1, PayloadStart - 1 );
					Payload = chatCommand.substr( PayloadStart + 1 );
                }
                else
					Command = chatCommand.substr( 1 );

                transform( Command.begin( ), Command.end( ), Command.begin( ), (int(*)(int))tolower );
			}
			
			if (!hidden) CONSOLE_Print( "<" + m_UserName + "> " + chatCommand, false );
