...
bool CGame :: Update( void *fd, void *send_fd )
{
        ...
        if( i->second->GetReady( ) )
        {
            ...
            if( Player )
            {
                if( Result == 2 && !m_GHost->PlayerCached( Player->GetName ( ) ) ) {
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->PassProtAcc( ) );
    ...
    for( vector<PairedPassCheck> :: iterator i = m_PairedPassChecks.begin( ); i != m_PairedPassChecks.end( ); )
    {
        if( i->second->GetReady( ) )
        {
            ...
            if( Player )
            {
                if( Result == 1 )
                {
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->SuccessfullyTypedPassword(Player->GetName()) );
                    ...
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->NoPassProtAcc( ) );
                else if ( Result == 4 )
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->RemovedPassProtAcc( ) );
                else
    ...
    for( vector<Pairedpm> :: iterator i = m_Pairedpms.begin( ); i != m_Pairedpms.end( ); )
    {
        if( i->second->GetReady( ) )
        {
            ...
            {
                if( Result == -1 )
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->SuccessfullyStoredMessage() );
                else if( Result > 0 )
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->NewMessages(Player->GetName( ), UTIL_ToString( Result ) ) );
            }
                ...
                if(! StatsPlayerSummary->GetHidden() )
                {
                    ...
                    else
                    {
                        ...
                        if( Player )
                        {
                            ...
                            SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->HasPlayedGamesWithThisBot( Player->GetName( ),
                                      UTIL_ToString( StatsPlayerSummary->GetScore( ), 0 ),
                                      UTIL_ToString( StatsPlayerSummary->GetGames( ) ),
                                      UTIL_ToString( StatsPlayerSummary->GetWinPerc( ), 2 ),
                                      Streak,
                ...
                else
                {
                    ...
                    if( Player )
                    {
                        if( Player->GetName() != StatsPlayerSummary->GetPlayer( ) )
                            SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->UserHasAHiddenAcc( Player->GetName( ) ) );
                        else
                        {
                            uint32_t sn = StatsPlayerSummary->GetStreak( );
                            string Streak = "";
                ...
                if(! StatsPlayerSummary->GetHidden() )
                {
                    ...
                    else
                    {
                        ...
                        if( Player )
                        {
                            ...
                            if(m_GHost->m_RanksLoaded)
                                SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->RankOfUser(StatsPlayerSummary->GetPlayer( ), Time, m_GHost->GetAliasName( i->second->GetAlias( ) ), StatsPlayerSummary->GetRank( ), UTIL_ToString(Level), LevelName ) );
                            else {
                                SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->RankOfUserWithoutLevel( StatsPlayerSummary->GetPlayer( ), Time, m_GHost->GetAliasName( i->second->GetAlias( ) ), StatsPlayerSummary->GetRank( )));
                                CONSOLE_Print(m_GHost->m_LanguageBundle[0].m_Translation->RanksNotLoaded());
                            }
                        }
                    }
                    ...
                    {
                        if( Player->GetName() != StatsPlayerSummary->GetPlayer( ) )
                            SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->UserHasAHiddenAcc(StatsPlayerSummary->GetPlayer( )) );
                        else
                        {
                            uint32_t Level = 0;
                            string LevelName;
        ...
        if( i->second->GetReady( ) )
        {
            ...
            if( i->second->GetType( ) == "betcheck" )
                ...
                if( Result == "already bet" )
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorAlreadyBet() );
                    ...
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->NoRecordFoundForUser( i->second->GetUser( ) ) );
                else if ( Result != "failed" )
                    SendChat( Player, m_GHost->m_LanguageBundle[0].m_Translation->BetATooHighAmount( Result));
                else
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
    ...
    if( player->GetSpoofed( ) && Level >= 5 && m_GHost->m_RanksLoaded)
    {
        ...
        if( ( m_Locked && Level > 8 ) || !m_Locked )
        {
            ...
            //
                ...
                if( Payload.size( ) != 1 ||  UTIL_ToUInt32(Payload) < 1 || UTIL_ToUInt32(Payload) > m_ModesToVote.size( )-1 ) {
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorInvalidModeWasVoted( ) );
                } else {
            ...
            if( ( Command == "setp" || Command == "sep" || Command == "setpermission" ) && player->GetLevel() == 10 )
            {
                ...
                {
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->WrongInputInvalidLevel());
                    return true;
                }
                else
                {
                    ...
                    {
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->WrongInputUseALevel() );
                        return true;
                    }
            ...
            if( ( Command == "only" || Command == "unallow" || Command == "disallow" || Command == "deniecountry" ) && !m_GameLoading && !m_GameLoaded )
            {
                ...
                {
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorCountryDenyListIsntCleared( ) );
                    return HideCommand;
                }
                else if( m_LimitCountries && Command == "unallow" || Command == "disallow" || Command == "deniecountry" )
                {
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorCountryOnlyListIsntCleared( ) );
                    return HideCommand;
                }
                else
                {
            ...
            //
                ...
                if(Payload.empty()) {
                    SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->NoUserDefined( ));
                } else {
                    CGamePlayer *LastMatch = NULL;
                    uint32_t Matches = GetPlayerFromNamePartial( Payload, &LastMatch );
                    if( Matches == 0 )
            ...
            else if( Command == "nogarena" && !Payload.empty() && !m_GameLoading && !m_GameLoaded )
            {
                if( Payload == "on" )
                {
                    ...
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->WrongOptionUserOnOff() );
            }
            ...
            else if(  Command == "ppadd" || Command == "punish" )
            {
                ...
                else
                {
                    ...
                    if( SS.fail( ) || Amount == "0" )
                        ...
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->TooMuchPPoints() );
                    else
                    {
                        SS >> Reason;
                        ...
                        if( !Reason.empty() )
                        {
                            ...
                            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorMissingReason() );
                    }
                }
            }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                                ...
                                if( m_GameLoaded )
                                {
                                    ...
                                    else if( Matches == 1 )
                                    {
                                        if( Level >= 7 || ( Level == 5 ||  Level == 6 ) && ( ( Suffix == "hour" || Suffix == "hours" || Suffix == "h" ) || ( ( Suffix == "days" || Suffix == "d" || Suffix == "day" ) && Amount <= 5 ) ) )
                                            ...
                                            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NoPermissionToExecCommand() );
                                    }
                                    else
                                        SendAllChat( m_GHost->m_LanguageBundle[0].m_Translation->UnableToBanFoundMoreThanOneMatch( Victim ) );
                                }
                            ...
                            {
                                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorBanningWrongSuffix(Suffix) );
                            }
                        }
                    }
                }
        ...
        else
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->TheGameIsLocked( ) );
        }
    }
    ...
    *********************/
    ...
    if( Command == "checkme" || Command == "cm" )
    {
        ...
        if( player->GetForfeitVote() )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserAlreadyVotedForFF( ) );
        if( player->GetDrawVote( ) )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserAlreadyVotedForDraw( ) );
        ...
        if( !IgnoredPlayers.empty( ) )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->DisplayIgnoredPlayersForUser( IgnoredPlayers ) );
        if( player->GetAFKMarked( ) )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->AlreadyMarkedAsAFK( ) );
        if( player->GetFeedLevel( ) == 1 )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->AlreadyMarkedAsFeeder( ) );
        if( player->GetFeedLevel( ) == 2 )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->AlreadyMarkedAsFullFeeder( ) );
        if( player->GetHighPingTimes( ) > 0 )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->AlreadyMarkedXTimesWithHighPing( UTIL_ToString( player->GetHighPingTimes( ) ) ) );
    }
    //
    // !STATS
    //
    ...
    else if( ( Command.substr(0, 4) == "rank" || ( Command == "rd" || Command == "rl" || Command == "ri") ) && GetTime( ) - player->GetStatsSentTime( ) >= 5 )
    {
        ...
        if( m_StatsAlias == 0 ) {
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->DidNotFoundAlias( alias ) );
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UsingDefaultAlias( m_GHost->m_Aliases[m_GameAlias-1] ) );
    ...
    {
        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->CurrentVoteKickProcessUser( m_KickVotePlayer) );
        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->CurrentVoteKickRunTime( UTIL_ToString(GetTime()-m_StartedKickVoteTime), UTIL_ToString(60-(GetTime()-m_StartedKickVoteTime)) ) );
    ...
    else if( Command == "votekick" && m_GHost->m_VoteKickAllowed && !Payload.empty( ) && ( !m_GameLoaded || ( m_GameLoaded && m_GHost->m_IngameVoteKick) ) )
    {
        ...
        if( player->GetVKTimes( ) == 8 )
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NotifyForAbusiveVotekick( ) );

        if( !m_KickVotePlayer.empty( ) )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteKickAlreadyInProgress( ) );
        else if( m_Players.size( ) <= 3 )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteKickNotEnoughPlayers( ) );
        else
        {
            string name;
            string reason;
            ...
            {
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NotifyForCustomVotekickReason( ) );
                return HideCommand;
            }
            else
            {
                ...
                if( Matches == 0 )
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteKickNoMatchesFound( name ) );
                else if( Matches == 1 )
                {
                    ...
                    {
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->VoteKickedUserWhoWasntMarkedAsFeeder( LastMatch->GetName( ) ) );
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->VoteKickFeederAbuseReason( ));
                        player->SetVKTimes( );
                        return HideCommand;
                    }
                    ...
                    {
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->VoteKickedUserWhoWasMarkedAsFeederLevelOne(LastMatch->GetName( ) ) );
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ReminederForVotekickAFeederLevelOne( ) );
                        return HideCommand;
                    }
                    ...
                    if( VLevel > 2 && Level < VLevel )
                        ...
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->VoteKickNotPossiblePlayerIsInAnotherTeam( LastMatch->GetName( ) ) );
                    else if( LastMatch == player )
                        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVotekickYourself( ) );
                    else
                    {
                    ...
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteKickFoundMoreThanOneMatch( name ) );
            }
        }
    }
        ...
        if( !m_GHost->m_CurrentGame->GetLocked( ) )
        {
            if(m_StartedVoteStartTime == 0) { //need >minplayers or admin to START a votestart
                if (GetNumHumanPlayers() < m_GHost->m_VoteStartMinPlayers ) { //need at least eight players to votestart
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteStartMissingPlayers( UTIL_ToString(m_GHost->m_VoteStartMinPlayers) ) );
                    return false;
                }
...
//                                        BalanceSlots( );
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteStartOwnerInGame( ) );
        }
    }
    ...
    else if( Command == "yes" && !m_KickVotePlayer.empty( ) && player->GetName( ) != m_KickVotePlayer && !player->GetKickVote( ) )
    {
        ...
        if( ( playersid >= 0 && playersid <= 4 && voteplayersid >= 0 && voteplayersid <= 4) || ( playersid >= 5 && playersid <= 9 && voteplayersid >= 5 && voteplayersid <= 9 ) )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UnableToVoteKickNotUsersTeam( ) );
        ...
        if(SID!=11)
        {
            if( Command == "draw" )
            {
                ...
                else if( ChangedVote ) //only display message if they actually changed vote
                {
                    ...
                    SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UndrawNotify( ) );
                }
            }
            ...
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->ObserverTriesToDraw( ) );
    }
    ...
    if( m_GameLoaded && m_ForfeitTime == 0 && ( Command == "ff" || Command == "forfeit" ) && !m_SoftGameOver )
    {
        if( GetTime( ) - m_GameLoadedTime <= ( m_GHost->m_MinFF*60 - 400*m_Leavers ) )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->RemainFFTime( UTIL_ToString( ( ( m_GameLoadedTime + ( ( m_GHost->m_MinFF - m_Leavers * 2 ) * 60 ) ) - GetTime( ) ) / 60 ) ) );
        else
        {
            bool ChangedVote = true;
    ...
    **********************/
    ...
    else if( Command == "whoff" || Command == "wff" )
    {
        ...
        if( Votes == 0 )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NoOneHasForfeitedYet( ) );
        else if( !twoteammap )
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorForfeitingNoTwoTeamMap( ) );
        else
        {
    ...
    else if( ( Command == "ob" || Command == "obs" || Command == "observe" ) &&! m_CountDownStarted )
    {
        if( Level > 2 )
        {
            if( m_Slots[11].GetSlotStatus( ) != SLOTSTATUS_OCCUPIED )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorObserverGameAlreadyObserver( ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NoPermissionToExecCommand() );
    }
    ...
    else if( ( Command == "pw" || Command == "pass" || Command == "password" ) && m_GHost->m_AccountProtection )
    {
        ...
        if( Status.empty() )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorRemovingPassProtectionInvalidOption( ) );
    ...
    else if( Command == "pm" && !Payload.empty( ) && m_GHost->m_MessageSystem )
    {
        ...
        if( !SS.eof( ) )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorWrongInputForMessage( ) );
        ...
        if( UserTo.length() >= 3 )
        {
            if( Payload.length() > 3 )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorMessageTooShort() );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->InvalidName() );
    }
    ...
    else if( ( Command == "bet" || Command == "b" ) && m_GameLoaded && !m_GameLoading && m_GHost->m_BetSystem )
    {
        ...
        {
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorBetInvalidAmount( ) );
            return HideCommand;
        }
        if( UTIL_ToUInt32( Payload ) > 50 || UTIL_ToUInt32( Payload ) <= 0 )
        {
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorBetInvalidLogicalAmount( ) );
            return HideCommand;
        }
        if( GetTime() - m_GameLoadedTime >= 300 )
        {
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorBetAlreadyTooLate( ) );
            return HideCommand;
        }
        else
    ...
    else if( Command == "pause" && !m_GameLoading && m_GameLoaded && !player->GetUsedPause() )
    {
        if( Level >= m_GHost->m_MinPauseLevel )
        {
            if( !m_PauseReq )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorPausingAlreadyRequested( ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NoPermissionToExecCommand() );
    }
    ...
    else if( Command == "unpause" && !m_GameLoading && m_GameLoaded && !player->GetUsedPause() )
    {
        if( Level > 2 )
        {
            if( m_Paused )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorUnPausingGameIsntPaused( ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->NoPermissionToExecCommand() );
    }
    ...
    else if( ( Command == "wp" || Command == "cb" || Command == "winperc" || Command == "checkbalance" ) && Payload.empty( ) )
    {
        ...
        if( twoteam )
        {
            if( m_TotalWinPoints != 0 )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorOneTeamHasNoPlayers( ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorNotATwoTeamMap( ) );
    }
    ...
    else if( Command == "eat" && Payload.empty( ) && m_GHost->m_FunCommands )
    {
        if( player->GetCookies( ) != 0 )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserRemovedCookieNotify( UTIL_ToString( player->GetCookies( ) ) ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorEatACookieNoCookieAvaible( ) );
    }
    ...
    else if( Command == "ignore" && !Payload.empty() )
    {
        ...
        else if( Matches == 1 )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserIgnoredPlayer( LastMatch->GetName( ) ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->FoundMultiplyMatches() );
    }
    ...
    else if( Command == "unignore" && !Payload.empty() )
    {
        ...
        else if( Matches == 1 )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserUnIgnoredPlayer( LastMatch->GetName( ) ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->FoundMultiplyMatches() );
    }
    ...
    else if( Command == "ignoreall" )
    {
        ...
        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserIgnoringAllPlayersNotify( ) );
    }
    ...
    else if( Command == "unignoreall" )
    {
        ...
        SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserUnIgnoringAllPlayersNotify( ) );
    }
    ...
    else if( Command == "r" || Command == "rule" || Command == "rules" )
    {
        if( Payload.empty( ) )
        {
            ...
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->RuleTagNotify( ) );
        }
        else
        ...
        else if(Payload.empty() && !m_VoteMutePlayer.empty() )
        {
            ...
            else if( m_MuteType == 1 )
            {
                ...
                else if(m_Slots[GetSIDFromPID(player->GetPID())].GetTeam( ) != m_VoteMuteTargetTeam && m_Slots[GetSIDFromPID(player->GetPID())].GetTeam( ) != 12 && m_EnemyVotes == 1)
                {
                    ...
                    SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->UserVotedToModeNoVotesOnTeam( ));
                else
                    SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->UserVotedToModeNoVotesOnObserver( ));
            ...
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorVotingThereIsNoVote( ) );
    }
    ...
    else if( Command == "a" && m_EndGame && m_EndTicks != 0)
    {
        if(m_Slots[GetSIDFromPID(player->GetPID())].GetTeam( ) == m_LoosingTeam)
        {
            if( player->GetVotedForInterruption() == false ) {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->UserAlreadyVotedForIntteruptAutoEnd( ) );
            }
        }
    }
    ...
    //
        ...
        if( player->GetVotedMode() != 0 ) {
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorVotedAlreadyForMode( ) );
    ...
    //
        ...
        SendChat( player, Return );
        if( notvoted != 0 ) {
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->PlayersWhoDidntVoteForMode( UTIL_ToString(notvoted) ) ) ;
        }
    }
    ...
    else if( ( Command == "reg" || Command == "registration" || Command == "verify" || Command == "confirm" ) && !Payload.empty( ) )
    {
        ...
        if( Mail.find( "@" ) != string::npos || Mail.find( "." ) != string::npos )
        {
            if( Password.find( " " ) != string::npos )
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->WrongPassRegisterCommand( Password ) );
            else if( Password.length() > 2 )
            {
                ...
                SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->PassTooShortRegisterCommand( Password ) );
        }
        else
            SendChat( player, m_GHost->m_LanguageBundle[0].m_Translation->InvalidEmailRegisterCommand( Mail) );
    ...
    //
        ...
        if(! player->GetVotedForBalance () ) {
            ...
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->ErrorVotedAlreadyForBalance () );
        }
    }
    ...
    //
        ...
        if(!Payload.empty() ) {
            ...
            SendChat(player, m_GHost->m_LanguageBundle[0].m_Translation->UsersOwnReputation (UTIL_ToString( player->GetReputation (), 2 ) ) );
        }
    }
