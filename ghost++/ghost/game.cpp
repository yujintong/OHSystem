...
CGame :: ~CGame( )
{
        ...
        if( m_CallableGameDBInit && m_CallableGameDBInit->GetReady( ) )
        {
                ...
                if( m_DatabaseID > 0 )
                {
                    CONSOLE_Print( "[GAME: " + m_GameName + "] Successfully created a dummy entry for the current running game." );
...
void CGame :: EventGameStarted( )
{
        ...
        m_CallableGameDBInit = m_GHost->m_DB->ThreadedGameDBInit( m_DBBans );
}
